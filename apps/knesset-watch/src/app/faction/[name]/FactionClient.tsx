...
export default function FactionClient({ data }: { data: FactionDetail }) {
  ...
  const bgColor = isCoalition === true ? 'bg-navy-soft/40' : isCoalition === false ? 'bg-accent-wash' : 'bg-surface';
  const statusLabel = isCoalition === true ? 'קואליציה' : isCoalition === false ? 'אופוזיציה' : 'לא ידוע';
  const statusColor = isCoalition === true ? 'bg-pass-wash text-pass' : isCoalition === false ? 'bg-accent-wash text-accent' : 'bg-surface text-ink-2';
    ...
    <div className="min-h-screen bg-paper" dir="rtl">
      <div className="max-w-4xl mx-auto px-6 py-8">
        <nav className="flex items-center gap-1 text-ui text-mute mb-6">
          ...
          <Link href="/mks" className="font-medium hover:text-ink transition-colors">ח"כים</Link>
          <span className="mx-1">›</span>
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            ...
            <span className={`text-meta font-medium px-2.5 py-1 rounded-full ${statusColor}`}>{statusLabel}</span>
          </div>
        </div>

        {/* Stats row */}
        <div className={`flex flex-wrap gap-6 rounded-card px-6 py-4 mb-8 ${bgColor}`}>
          ...
          <div className="flex flex-col gap-1.5">
            {currentMks.map(mk => (
              <Link
                ...
                className={`flex items-center gap-3 rounded-card px-4 py-3 transition-colors ${bgColor} hover:brightness-95`}
              >
                <span className="font-medium text-ui">{mk.firstName} {mk.lastName}</span>
            ...
            <div className="flex flex-col gap-1.5">
              {formerMks.map(mk => (
                <Link
                  key={mk.personId}
                  href={`/mk/${mk.slug ?? mk.personId}`}
                  className="flex items-center gap-3 rounded-card px-4 py-3 bg-surface hover:bg-surface-2 transition-colors"
                >
                  <span className="font-medium text-ui text-mute">{mk.firstName} {mk.lastName}</span>
                  <svg className="w-3.5 h-3.5 text-mute shrink-0 mr-auto rotate-180" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2.5">
                    <path d="m6 3 5 5-5 5"/>
                  </svg>
                </Link>
