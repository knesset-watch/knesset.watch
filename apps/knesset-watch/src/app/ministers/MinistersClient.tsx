...
export default function MinistersClient({ ministers }: { ministers: MinisterInfo[] }) {
  ...
  function MinisterRow({ m }: { m: MinisterInfo }) {
    return (
      <div
        className={`flex flex-col sm:grid sm:grid-cols-[1fr_1fr_5rem_5rem_5rem] gap-2 sm:gap-4 py-3 px-4 rounded-card sm:items-center transition-all cursor-pointer border border-transparent hover:border-line ${
          m.isCoalition ? 'bg-navy-soft/40 hover:bg-navy-soft/70' : 'bg-surface hover:bg-surface-2'
        }`}
      >
        <div className="flex flex-col">
          <Link href={`/mk/${m.slug ?? m.id}`} className="font-medium text-body hover:underline">
            {m.name}
          </Link>
        ...
        <div className="min-w-0">
          {m.ministry && m.officeSlug ? (
            <Link
              ...
              className="text-meta text-ink-2 leading-snug line-clamp-2 hover:underline hover:text-accent transition-colors"
            >
              {m.ministerRole}
            </Link>
          ) : m.ministry ? (
            ...
            <span className="text-meta text-ink-2 leading-snug line-clamp-2">{m.ministerRole}</span>
          )}
        </div>
        <div className="flex flex-col items-center">
          <span className="text-section font-medium">{m.committeeSessionCount}</span>
          {m.committeeSessionCount > 0 && (
            <div className="w-full bg-line rounded-full h-0.5 mt-1">
              <div
        ...
        <span className="text-section font-medium text-center">{m.billCount}</span>
        <span className="text-section font-medium text-center">{m.passedCount > 0 ? m.passedCount : '—'}</span>
      </div>
    );
  }
    ...
    return (
      <button
        ...
        className={`text-meta font-medium transition-colors flex items-center gap-1 whitespace-nowrap ${className} ${
          isActive ? 'text-ink bg-accent-wash px-2 py-1 rounded' : 'text-mute hover:text-ink-2'
        }`}
      >
        {label}{arrow}
      </button>
    ...
    <div className="min-h-screen bg-paper" dir="rtl">
      <div className="max-w-4xl mx-auto px-6 py-8">
        <nav className="flex items-center gap-1 text-ui text-mute mb-6">
          ...
          <span className="text-ink font-medium">שרים</span>
        </nav>

        <h1 className="text-4xl font-medium mb-1">שרים</h1>
        <p className="text-ui text-mute mb-8">חברי כנסת המכהנים גם כשרים בממשלה</p>
        ...
        <div className="rounded-card border border-line-soft px-4 py-3 bg-surface text-meta text-mute mb-6">
