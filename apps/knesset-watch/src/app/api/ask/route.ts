...
6. גבולות הרשומה: משפט אחד על מה שאינו מכוסה בנתוני הכנסת — למשל הצהרות לתקשורת, ראיונות, פגישות ממשלתיות.
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [speakerTurns, plenaryTurns, vectorResults, votes, bills, queries] = await Promise.all([
      speakerTurnsPromise,
      plenaryTurnsPromise,
      vectorSearchPromise,
    ...
    if (votes.length > 0) {
      ...
      const factionVoteCtx = (factionId && votes.length > 0)
    ...
    // 7. Call Gemini — Knesset data only, no external sources
