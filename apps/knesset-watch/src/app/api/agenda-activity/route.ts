import { NextResponse } from "next/server";
import { validateApiAuth } from "@/lib/ui/auth-utils";
import Database from "better-sqlite3";
import { dbAvailable, dbPath } from "@/lib/knesset-db";
import {
  computeAgendaActivity,
  type AgendaSelection,
} from "@/lib/agenda-activity";
import { POLITICAL_ISSUES } from "@/lib/canonical-agendas";
import {
  getMkProfile,
  occupationLine,
  educationLine,
  tenureLabel,
} from "@/lib/mk-profiles";

/**
 * תקרה שמונעת בקשה שתסרוק את כל האג'נדות בבת אחת.
 *
 * הועלתה מ-10 ל-45. הטקסונומיה גדלה מ-12 סוגיות ל-166 צירים, ושישה
 * אשכולות יכולים להחזיק עד 30 שאלות — בדיוק על התקרה הקודמת. המרווח
 * נועד למנוע שגיאה שמופיעה רק אחרי שהמשתמשת כבר ענתה על הכל, וזו
 * שגיאה שנקראת כתקלה ולא כמגבלה.
 */
const MAX_SELECTIONS = 45;

/** כמה ח"כים מוחזרים. התצוגה מראה עשרה, השאר לגלילה */
const RESULT_LIMIT = 25;

function parseSelections(
  raw: unknown,
): { selections: AgendaSelection[] } | { error: string } {
  if (!Array.isArray(raw) || raw.length === 0) {
    return { error: "נדרש מערך selections עם אג'נדה אחת לפחות" };
  }
  if (raw.length > MAX_SELECTIONS) {
    return { error: `עד ${MAX_SELECTIONS} אג'נדות בבקשה אחת` };
  }

  const selections: AgendaSelection[] = [];

  for (const item of raw) {
    if (typeof item !== "object" || item === null) {
      return { error: "כל בחירה צריכה להיות אובייקט עם issueId" };
    }
    const { issueId, stanceId } = item as {
      issueId?: unknown;
      stanceId?: unknown;
    };

    if (typeof issueId !== "string") {
      return { error: "issueId חסר או אינו מחרוזת" };
    }
    const issue = POLITICAL_ISSUES.find((i) => i.id === issueId);
    if (!issue) {
      return { error: `אג'נדה לא מוכרת: ${issueId}` };
    }

    if (stanceId !== undefined) {
      if (
        typeof stanceId !== "string" ||
        !issue.stances.some((s) => s.id === stanceId)
      ) {
        return {
          error: `עמדה לא מוכרת באג'נדה ${issueId}: ${String(stanceId)}`,
        };
      }
    }

    selections.push({
      issueId,
      stanceId: typeof stanceId === "string" ? stanceId : undefined,
    });
  }

  return { selections };
}

interface CommitteeSummary {
  committeeName: string;
  sessionCount: number;
}

interface CommitteeActivitySummary {
  attendanceCount: number;
  topCommittees: CommitteeSummary[];
}

function getCommitteeActivityForMks(
  mkIds: number[],
): Map<number, CommitteeActivitySummary> {
  const result = new Map<number, CommitteeActivitySummary>();

  if (mkIds.length === 0) {
    return result;
  }

  const databasePath = dbPath();

  if (!databasePath) {
    return result;
  }

  const db = new Database(databasePath, {
    readonly: true,
  });

  try {
    const placeholders = mkIds.map(() => "?").join(",");

    const attendanceRows = db
      .prepare(
        `
        SELECT
          mk_id,
          COUNT(DISTINCT session_id) AS attendance_count
        FROM committee_attendance
        WHERE mk_id IN (${placeholders})
        GROUP BY mk_id
        `,
      )
      .all(...mkIds) as Array<{
      mk_id: number;
      attendance_count: number;
    }>;

    const committeeRows = db
      .prepare(
        `
        SELECT
          ca.mk_id,
          cs.committee_name,
          COUNT(DISTINCT ca.session_id) AS session_count
        FROM committee_attendance ca
        INNER JOIN committee_session cs
          ON cs.id = ca.session_id
        WHERE
          ca.mk_id IN (${placeholders})
          AND cs.committee_name IS NOT NULL
          AND TRIM(cs.committee_name) <> ''
        GROUP BY
          ca.mk_id,
          cs.committee_name
        ORDER BY
          ca.mk_id,
          session_count DESC
        `,
      )
      .all(...mkIds) as Array<{
      mk_id: number;
      committee_name: string;
      session_count: number;
    }>;

    for (const mkId of mkIds) {
      result.set(mkId, {
        attendanceCount: 0,
        topCommittees: [],
      });
    }

    for (const row of attendanceRows) {
      const existing = result.get(row.mk_id);

      if (existing) {
        existing.attendanceCount = row.attendance_count;
      }
    }

    for (const row of committeeRows) {
      const existing = result.get(row.mk_id);

      if (!existing) {
        continue;
      }

      if (existing.topCommittees.length >= 3) {
        continue;
      }

      existing.topCommittees.push({
        committeeName: row.committee_name,
        sessionCount: row.session_count,
      });
    }

    return result;
  } finally {
    db.close();
  }
}

export async function POST(request: Request) {
  const authError = await validateApiAuth(
    "SITE_PASSWORD",
    "knesset-watch_auth_token",
  );
  if (authError) return authError;

  if (!dbAvailable()) {
    return NextResponse.json(
      { error: "Database not available" },
      { status: 503 },
    );
  }

  let body: unknown;
  try {
    body = await request.json();
  } catch {
    return NextResponse.json(
      { error: "גוף הבקשה אינו JSON תקין" },
      { status: 400 },
    );
  }

  const parsed = parseSelections(
    (body as { selections?: unknown })?.selections,
  );
  if ("error" in parsed) {
    return NextResponse.json({ error: parsed.error }, { status: 400 });
  }

  try {
    const result = computeAgendaActivity(parsed.selections);
    const limitedRows = result.rows.slice(0, RESULT_LIMIT);

    const committeeActivity = getCommitteeActivityForMks(
      limitedRows.map((row) => row.mkId),
    );
    // תמונה ושורת רקע מ-Wikidata. חסרים אצל 7 ח"כים — התצוגה נופלת
    // חזרה לראשי תיבות, כפי שהיה קודם.
    const rows = limitedRows.map((row) => {
      const profile = getMkProfile(row.mkId);
      const committees = committeeActivity.get(row.mkId);

      return {
        ...row,
        photo: profile?.photo ?? null,
        occupation: occupationLine(profile),
        education: educationLine(profile),
        tenure: tenureLabel(profile),

        committeeAttendanceCount: committees?.attendanceCount ?? 0,

        topCommittees: committees?.topCommittees ?? [],
      };
    });

    return NextResponse.json({
      rows,
      // כמה ח"כים בכלל נכנסו לדירוג, לפני החיתוך
      totalRanked: result.rows.length,
      coverage: result.coverage,
      generatedAt: result.generatedAt,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "שגיאה לא ידועה";
    console.error("Agenda activity error:", message);
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
